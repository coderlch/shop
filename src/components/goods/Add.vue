<template>
  <div>
    ...
    <!--图片预览对话框-->
    <el-dialog title='图片预览' :visible.sync='picPreviewDialogVisible' width='40%'>
      <img class='picPreview' :src='picURL' alt='图片预览'>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      goodsCateList: [],
      // 分类列表标签
      cascaderProps: {label: 'cat_name', value: 'cat_id', children: 'children'},
      // 动态参数数据列表
      manyTabData: [],
      // 静态参数列表
      onlyTabData: [],
      ...
      // 图片上传组件请求头对象设置
      headersRes: {Authorization: window.sessionStorage.getItem('token')},
      // 图片预览对话框显示与隐藏
